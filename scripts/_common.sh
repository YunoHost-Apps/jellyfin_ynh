...
case "$(lsb_release --codename --short)" in
    ...
    *) echo "Unknown release: $(lsb_release --codename --short)" >&2; exit 1 ;;
esac
case "$YNH_ARCH" in
    arm64) : ;;
    armhf) : ;;
...
install_jellyfin_packages() {
    ...
    for pkg in web server; do
        src_url=$(grep 'SOURCE_URL=' "$YNH_APP_BASEDIR/conf/$pkg.$(lsb_release --codename --short).$YNH_ARCH.src" | cut -d= -f2-)
        if ! curl --output /dev/null --silent --head --fail "$src_url"; then
            ynh_replace_string \
                ...
                --target_file="$YNH_APP_BASEDIR/conf/$pkg.$(lsb_release --codename --short).$YNH_ARCH.src"
        fi
    done
    ...
    ynh_setup_source --dest_dir=$tempdir --source_id="ffmpeg.$(lsb_release --codename --short).$YNH_ARCH"
    ynh_setup_source --dest_dir=$tempdir --source_id="server.$(lsb_release --codename --short).$YNH_ARCH"
    ynh_setup_source --dest_dir=$tempdir --source_id="web.$(lsb_release --codename --short).$YNH_ARCH"
    ...
    ynh_install_app_dependencies \
        jellyfin-ffmpeg5="$ffmpeg_pkg_version-$(lsb_release --codename --short)" \
